...
# Smoke-test the relay-only embedded image from an empty volume. The test is
# deliberately image-based: it catches missing runtime files, permissions,
# SQLite bootstrap failures, readiness regressions, restart key rotation, and
# backup/restore mistakes that Rust unit tests cannot observe.
