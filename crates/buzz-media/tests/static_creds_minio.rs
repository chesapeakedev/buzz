...
use buzz_media::storage::{BlobMeta, BlobStorage, MediaStorage};
...
async fn static_creds_round_trip_against_minio() {
    ...
    run_blob_storage_contract(&storage).await;
}

async fn run_blob_storage_contract(storage: &dyn BlobStorage) {
    let run = Uuid::new_v4();
    ...
    storage
        .delete(&MediaStorage::ctx_sidecar_key(&tenant_a, &sha))
        .await
