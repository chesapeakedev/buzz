...
impl Db {
    ...
    pub async fn try_lock_usage_metrics(
        ...
        let mut connection =
            observability::acquire(&self.postgres().pool, observability::PoolRole::Writer).await?;
    ...
    ///
    /// SQLite dispatches to its backend implementation; PostgreSQL preserves
    /// ordering watermarks and hard-delete semantics for protected coordinates.
